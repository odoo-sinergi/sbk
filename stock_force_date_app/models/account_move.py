        ...
        for vals in vals_list:
            if self.env.user.has_group('stock_force_date_app.group_stock_force_date'):
                if vals.get('stock_move_id',False):
                    ...
                    if stock_move.picking_id.force_date:
                        self._cr.execute("select cast(force_date at time zone 'UTC' as date) as force_date from stock_picking where id=%s"%(stock_move.picking_id.id))
                        force_date = stock_move.picking_id.force_date.strftime('%Y-%m-%d') #self._cr.fetchone()[0]
                        vals['date'] = force_date or fields.Date.today()
                if self._context.get('accounting_date', False):
                    vals['date'] = self._context.get('accounting_date', False) or fields.Date.today()
        action_create_entry = super(AccountMove, self).create(vals_list)
